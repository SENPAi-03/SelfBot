...
def startprint():
    ...
    print(f'''{Fore.CYAN}
    ᏕᏋᏁᎮᏗᎥ
                       {Fore.CYAN}SelfBot v{SELFBOT.__version__} | {Fore.CYAN}Logged in as: {SelfBot.user.name}#{SelfBot.user.discriminator} {Fore.CYAN}| ID: {Fore.CYAN}{SelfBot.user.id}   
                       {Fore.CYAN}Nitro Sniper | {Fore.CYAN}{nitro}
                       {Fore.CYAN}Cached Users: {Fore.CYAN}{len(SelfBot.users)}
                       {Fore.CYAN}Guilds: {Fore.CYAN}{len(SelfBot.guilds)}
                       {Fore.CYAN}Prefix: {Fore.CYAN}{SelfBot.command_prefix}
    ''' + Fore.RESET)
...
def Init():
    token = config.get('token')
    try:
        SelfBot.run(token, bot=False, reconnect=True)
        os.system(f'title (SelfBot Selfbot) - Version {SELFBOT.__version__}')
...
SelfBot = commands.Bot(description='SelfBot Selfbot', command_prefix=prefix, self_bot=True)

SelfBot.antiraid = False
SelfBot.msgsniper = True
SelfBot.slotbot_sniper = True
SelfBot.giveaway_sniper = True
SelfBot.mee6 = False
SelfBot.mee6_channel = None
SelfBot.yui_kiss_user = None
SelfBot.yui_kiss_channel = None
SelfBot.yui_hug_user = None
SelfBot.yui_hug_channel = None
SelfBot.sniped_message_dict = {}
SelfBot.sniped_edited_message_dict = {}
SelfBot.whitelisted_users = {}
SelfBot.copycat = None
...
async def on_message_edit(before, after):
    await SelfBot.process_commands(after)


@SelfBot.event
async def on_message(message):
    if SelfBot.copycat is not None and SelfBot.copycat.id == message.author.id:
    ...
    if f'Congratulations <@{SelfBot.user.id}>' in message.content:
        if SelfBot.giveaway_sniper:
            if message.author.id == 294882584201003009:
                print(""
...
async def on_member_ban(guild: discord.Guild, user: discord.user):
    if SelfBot.antiraid is True:
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in SelfBot.whitelisted_users.keys() and i.user.id in SelfBot.whitelisted_users[
                    guild.id].keys() and i.user.id is not SelfBot.user.id:
...
@SelfBot.command(aliases=["queue"])
async def play(ctx, *, query):
    await ctx.message.delete()
    voice = get(SelfBot.voice_clients, guild=ctx.guild)
...
@SelfBot.command(aliases=["lyric"])
async def lyrics(ctx, *, args):
    await ctx.message.delete()
    await ctx.send("Showing lyrics for " + args)


@SelfBot.command(aliases=[])
...
@SelfBot.command(aliases=['ar', 'antiraid'])
...
@SelfBot.command(aliases=['wl'])
async def whitelist(ctx, user: discord.Member = None):
    await ctx.message.delete()
    if user is None:
        await ctx.send("Please specify a user to whitelist")
    else:
        if ctx.guild.id not in SelfBot.whitelisted_users.keys():
            SelfBot.whitelisted_users[ctx.guild.id] = {}
        if user.id in SelfBot.whitelisted_users[ctx.guild.id]:
    ...
    #         await ctx.send('That user is already whitelisted')
    #     else:
    #         SelfBot.whitelisted_users[ctx.guild.id][user.id] = 0
    #         await ctx.send("Whitelisted **" + user.name.replace("*", "\*").replace("`", "\`").replace("_","\_") + "#" + user.discriminator + "**")


@SelfBot.command(aliases=['wld'])
async def whitelisted(ctx, g=None):
    await ctx.message.delete()
    if g == '-g' or g == '-global':
        whitelist = '`All Whitelisted Users:`\n'
        for key in SelfBot.whitelisted_users:
            for key2 in SelfBot.whitelisted_users[key]:
                user = SelfBot.get_user(key2)
        ...
        for key in SelfBot.whitelisted_users:
            if key == ctx.guild.id:
                for key2 in SelfBot.whitelisted_users[ctx.guild.id]:
                    user = SelfBot.get_user(key2)
                    whitelist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
...
@SelfBot.command(aliases=['clearwl', 'clearwld'])
async def clearwhitelist(ctx):
    await ctx.message.delete()
    SelfBot.whitelisted_users.clear()
...
async def yuikiss(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        SelfBot.yui_kiss_user = user.id
        SelfBot.yui_kiss_channel = ctx.channel.id
        if SelfBot.yui_kiss_user is None or SelfBot.yui_kiss_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while SelfBot.yui_kiss_user is not None and SelfBot.yui_kiss_channel is not None:
            await SelfBot.get_channel(SelfBot.yui_kiss_channel).send('yui kiss ' + str(SelfBot.yui_kiss_user), delete_after=0.1)
...
async def yuihug(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        SelfBot.yui_hug_user = user.id
        SelfBot.yui_hug_channel = ctx.channel.id
        if SelfBot.yui_hug_user is None or SelfBot.yui_hug_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
...
@SelfBot.command(aliases=["automee6"])
async def mee6(ctx, param=None):
    ...
    while SelfBot.mee6 is True:
        ...
        await SelfBot.get_channel(SelfBot.mee6_channel).send(random.choice(sentences), delete_after=0.1)
        await asyncio.sleep(60)


@SelfBot.command(aliases=['slotsniper', "slotbotsniper"])
async def slotbot(ctx, param=None):
    await ctx.message.delete()
    SelfBot.slotbot_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        SelfBot.slotbot_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        SelfBot.slotbot_sniper = False
...
@SelfBot.command(aliases=['giveawaysniper'])
async def giveaway(ctx, param=None):
    await ctx.message.delete()
    SelfBot.giveaway_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        SelfBot.giveaway_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        SelfBot.giveaway_sniper = False
        ...
        if isinstance(message.channel, discord.DMChannel) or isinstance(message.channel, discord.GroupChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    message.content) + "\n\n**Attachments:**\n" + links
                await message.channel.send(message_content)
    if len(SelfBot.sniped_message_dict) > 1000:
        SelfBot.sniped_message_dict.clear()
    attachments = message.attachments
    if len(attachments) == 0:
        channel_id = message.channel.id
        message_content = "`" + str(discord.utils.escape_markdown(str(message.author))) + "`: " + str(
            message.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        SelfBot.sniped_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
@SelfBot.event
async def on_message_edit(before, after):
    if before.author.id == SelfBot.user.id:
        return
    if SelfBot.msgsniper:
        ...
        if isinstance(before.channel, discord.DMChannel) or isinstance(before.channel, discord.GroupChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    before.content) + "\n\n**Attachments:**\n" + links
                await before.channel.send(message_content)
    if len(SelfBot.sniped_edited_message_dict) > 1000:
        SelfBot.sniped_edited_message_dict.clear()
    attachments = before.attachments
    if len(attachments) == 0:
        ...
        message_content = "`" + str(discord.utils.escape_markdown(str(before.author))) + "`: \n**BEFORE**\n" + str(
            before.content).replace("@everyone", "@\u200beveryone").replace("@here",
                                                                            "@\u200bhere") + "\n**AFTER**\n" + str(
            after.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        SelfBot.sniped_edited_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
async def snipe(ctx):
    ...
    if currentChannel in SelfBot.sniped_message_dict:
        await ctx.send(SelfBot.sniped_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!")


@SelfBot.command(aliases=["esnipe"])
async def editsnipe(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in SelfBot.sniped_edited_message_dict:
        await ctx.send(SelfBot.sniped_edited_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!")
...
async def help(ctx, category=None):
    await ctx.message.delete()
    if category is None:
        embed = discord.Embed(color=0x404040, timestamp=ctx.message.created_at)
        embed.set_author(name="𝗦𝗘𝗡𝗣𝗔𝗶 𝗦𝗘𝗟𝗙𝗕𝗢𝗧 | 𝙋𝙍𝙀𝙁𝙄𝙓: " + str(SelfBot.command_prefix),
                         icon_url=SelfBot.user.avatar_url)
        embed.set_thumbnail(url=SelfBot.user.avatar_url)
        embed.set_image(url="https://media.discordapp.net/attachments/890520706494894090/890526626545279026/SPOILER_20210923_121159.gif?width=320&height=180")
        embed.add_field(name="\uD83E\uDDCA `SelfBot`", value="Shows all SelfBot commands", inline=False)
        ...
        embed = discord.Embed(color=random.randrange(0x1000000), timestamp=ctx.message.created_at)
        embed.set_image(
            url="https://media.discordapp.net/attachments/890520706494894090/890526626545279026/SPOILER_20210923_121159.gif?width=432&height=265")
...
@SelfBot.command(aliases=["giphy", "tenor", "searchgif"])
...
@SelfBot.command(aliases=["img", "searchimg", "searchimage", "imagesearch", "imgsearch"])
...
@SelfBot.command(aliases=["addemoji", "stealemote", "addemote"])
...
@SelfBot.command(aliases=["stopcopycatuser", "stopcopyuser", "stopcopy"])
async def stopcopycat(ctx):
    await ctx.message.delete()
    if SelfBot.user is None:
        await ctx.send("You weren't copying anyone to begin with")
        return
    await ctx.send("Stopped copying " + str(SelfBot.copycat))
    SelfBot.copycat = None


@SelfBot.command(aliases=["copycatuser", "copyuser"])
async def copycat(ctx, user: discord.User):
    await ctx.message.delete()
    SelfBot.copycat = user
    await ctx.send("Now copying " + str(SelfBot.copycat))


@SelfBot.command(aliases=["9/11", "911", "terrorist"])
...
@SelfBot.command(aliases=["jerkoff", "ejaculate", "orgasm"])
async def cum(ctx):
    await ctx.message.delete()
...
@SelfBot.command(aliases=["spamchangegcname", "changegcname"])
async def spamgcname(ctx):
    await ctx.message.delete()
    if isinstance(ctx.message.channel, discord.GroupChannel):
        watermark = "SelfBot LOL"
        name = ""
...
@SelfBot.command(aliases=["fakename"])
...
@SelfBot.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
...
@SelfBot.command(aliases=["distort"])
...
@SelfBot.command(aliases=['markasread', 'ack'])
async def read(ctx):
    await ctx.message.delete()
    for guild in SelfBot.guilds:
        await guild.ack()
...
@SelfBot.command(aliases=["deepfry"])
...
@SelfBot.command(aliases=["pornhublogo", "phlogo"])
...
@SelfBot.command(aliases=["pornhubcomment", 'phc'])
...
@SelfBot.command(aliases=["reversesearch", "anticatfish", "catfish"])
async def revav(ctx, user: discord.Member = None):
    await ctx.message.delete()
    if user is None:
        user = ctx.author
    try:
        em = discord.Embed(description=f"[{user.name}](https://images.google.com/searchbyimage?image_url={user.avatar_url})'s Avatar on Google")
        await ctx.send(embed=em)
    except Exception as e:
        print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@SelfBot.command(aliases=['pfp', 'avatar'])
...
@SelfBot.command(aliases=["del", "quickdel"])
...
@SelfBot.command(name='1337speak', aliases=['leetspeak'])
...
@SelfBot.command(aliases=['pfpget', 'stealpfp'])
...
@SelfBot.command(name='set-pfp', aliases=['setpfp', 'pfpset,"changepfp'])
async def _set_pfp(ctx, *, url):
    ...
    try:
        ...
        with open('Images/Avatars/PFP-1.png', 'rb') as f:
            await SelfBot.user.edit(password=password, avatar=f.read())
    except discord.HTTPException as e:
        print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@SelfBot.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
@SelfBot.command(aliases=['dong', 'penis'])
...
@SelfBot.command(aliases=['changehypesquad'])
...
@SelfBot.command(aliases=['tokenfucker', 'disable', 'crash'])
...
@SelfBot.command(aliases=['fakeconnection', 'spoofconnection', 'spoofcon', "fakecon"])
...
@SelfBot.command(aliases=['tokinfo', 'tdox'])
...
@SelfBot.command(aliases=["copyguild", "copyserver"])
async def copy(ctx):  # b'\xfc'
    await ctx.message.delete()
    await SelfBot.create_guild(f'backup-{ctx.guild.name}')
...
@SelfBot.command(aliases=["rekt", "nuke"])
async def destroy(ctx):
    ...
    try:
        ...
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="SelfBot")
    for _i in range(250):
        await ctx.guild.create_role(name="SelfBot", color=RandomColor())


@SelfBot.command(aliases=["banwave", "banall", "etb"])
async def massban(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.ban(reason="SelfBot")
        except:
            pass
...
@SelfBot.command(aliases=["kickall", "kickwave"])
async def masskick(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.kick(reason="SelfBot")
        except:
            pass


@SelfBot.command(aliases=["spamroles"])
...
@SelfBot.command(aliases=["masschannels", "masschannel", "ctc"])
...
@SelfBot.command(aliases=["delchannel"])
...
@SelfBot.command(aliases=["deleteroles"])
...
@SelfBot.command(aliases=["purgebans", "unbanall"])
...
@SelfBot.command(name='get-color', aliases=['color', 'colour', 'sc', "hexcolor", "rgb"])
...
@SelfBot.command(aliases=['rainbowrole'])
...
@SelfBot.command(aliases=["guildinfo"])
...
@SelfBot.command(aliases=['guildpfp', 'serverpfp', 'servericon'])
...
@SelfBot.command(aliases=['serverbanner'])
...
@SelfBot.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
async def _first_message(ctx, channel: discord.TextChannel = None):
    await ctx.message.delete()
    if channel is None:
        channel = ctx.channel
    first_message = (await channel.history(limit=1, oldest_first=True).flatten())[0]
    embed = discord.Embed(description=f"First Message\n[Jump]({first_message.jump_url})")
    await ctx.send(embed=embed)
...
@SelfBot.command(aliases=["renameserver", "nameserver"])
...
async def prefix(ctx, prefix):
    await ctx.message.delete()
    SelfBot.command_prefix = str(prefix)
...
@SelfBot.command(aliases=['bitcoin'])
async def btc(ctx):
    await ctx.message.delete()
...
@SelfBot.command(pass_context=True, aliases=["cyclename", "autoname", "autonick", "cycle"])
...
@SelfBot.command(aliases=["stopcyclename", "cyclestop", "stopautoname", "stopautonick", "stopcycle"])
...
async def clearblocked(ctx):
    await ctx.message.delete()
    print(SelfBot.user.relationships)
    for relationship in SelfBot.user.relationships:
        if relationship is discord.RelationshipType.blocked:
            print(relationship)
            await relationship.delete()


@SelfBot.command(aliases=["changeregions", "changeregion", "regionschange"])
...
@SelfBot.command(aliases=["gcleave"])
...
@SelfBot.command(aliases=["vagina"])
async def pussy(ctx):
    await ctx.message.delete()
...
async def purge(ctx, amount: int):
    await ctx.message.delete()
    async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == SelfBot.user).map(
            lambda m: m):
        try:
            await message.delete()
        except:
...
@SelfBot.command(name='group-leaver',
                aliase=['leaveallgroups', 'leavegroup', 'leavegroups', "groupleave", "groupleaver"])
async def _group_leaver(ctx):
    await ctx.message.delete()
    for channel in SelfBot.private_channels:
        if isinstance(channel, discord.GroupChannel):
            await channel.leave()


@SelfBot.command(aliases=["streaming"])
async def stream(ctx, *, message):
    ...
    await SelfBot.change_presence(activity=stream)


@SelfBot.command(alises=["game"])
async def playing(ctx, *, message):
    await ctx.message.delete()
    game = discord.Game(
        name=message
    )
    await SelfBot.change_presence(activity=game)


@SelfBot.command(aliases=["listen"])
async def listening(ctx, *, message):
    await ctx.message.delete()
    await SelfBot.change_presence(
...
@SelfBot.command(aliases=["stopstreaming", "stopstatus", "stoplistening", "stopplaying", "stopwatching"])
async def stopactivity(ctx):
    await ctx.message.delete()
    await SelfBot.change_presence(activity=None, status=discord.Status.dnd)
...
@SelfBot.command(aliases=["fliptable"])
...
@SelfBot.command(name='rolecolor')
...
@SelfBot.command(aliases=["logout"])
async def shutdown(ctx):
    await ctx.message.delete()
    await SelfBot.logout()


@SelfBot.command(aliases=["nitrogen"])
async def nitro(ctx):
    await ctx.message.delete()
